...
function renderCars(){const b=document.getElementById('cars-body');b.innerHTML='';DATA.cars.forEach(c=>{const p=carProfit(c);const ps=carPhotos(c);const thumb=ps.length?`<img src="${esc(ps[0])}" alt="" style="height:30px;width:46px;object-fit:cover;border-radius:5px;vertical-align:middle;margin-right:7px">`:'';const more=ps.length>1?` <span style="font-size:11px;color:var(--faint)">+${ps.length-1}</span>`:'';const pub=`<button class="btn-ghost btn-sm" onclick="togglePublish('${c.id}')" title="Публикация на лендинге" style="${c.publish?'background:var(--accent);color:#fff;border-color:var(--accent)':''}">🌐 ${c.publish?'на сайте':'скрыто'}</button>`;const tr=document.createElement('tr');tr.innerHTML=`<td>${thumb}${esc(c.name)}${more}</td><td>${esc(c.country||'')}</td><td>${esc(c.date||'')}</td><td class="num">${fmt(c.purchase)}</td><td class="num">${fmt(c.delivery)}</td><td class="num">${fmt(c.customs)}</td><td class="num">${fmt(c.util)}</td><td class="num">${fmt(c.other)}</td><td class="num">${fmt(carCosts(c))}</td><td class="num">${fmt(c.sale)}</td><td class="num ${p>=0?'profit-pos':'profit-neg'}">${fmt(p)}</td><td style="white-space:nowrap">${pub} <button class="btn-ghost btn-sm" onclick="editCar('${c.id}')">✎</button> <button class="btn-ghost btn-sm" onclick="genAds('${c.id}')">✨ Текст${c.ads&&(c.ads.telegram||c.ads.vk||c.ads.instagram)?' ✓':''}</button> <button class="del" onclick="delCar('${c.id}')">✕</button></td>`;b.appendChild(tr);});renderSummary();}
