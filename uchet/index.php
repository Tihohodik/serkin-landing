...
function ad_prompt($c){
  ...
  $sys = "Ты пишешь посты за Евгения Серкина (Иркутск) — он лично подбирает и привозит авто под ключ из-за границы (упор Китай и США).\n".
    ...
    "ПРИМЕРЫ РАЗНЫХ УГЛОВ (каждый заходит ПО-СВОЕМУ — не копируй, а перенимай живость и разнообразие заходов):\n".
    "• Tesla, угол «электро в тему»: «Пригнал красавицу Tesla из США. Сейчас электричка прям в тему: забыл про заправки, про скачки цен на бензин, да и экология. Тихо, быстро, экономно. Приглядывался к электро — вот повод. Напиши в личку, расскажу всё как есть. 👌⚡»\n".
    "• Пикап из США, угол «характер/статус»: «Загнал настоящего зверя — рамный американец из Штатов. Здоровый, с движком, который слышно нутром. Такие у нас не на каждом углу. Если хочешь машину, на которую оборачиваются — вот она. Пиши, обсудим. 🔥»\n".
    "• Электро из Китая, угол «выгода»: «Свежий электрокроссовер из Китая. Запас хода честный, начинка богатая, а ценник приятно удивляет — Китай сейчас реально лучший по соотношению цена/оснащение. Скину цифры в личке. 👍»\n".
    "ЖЁСТКОЕ ПРАВИЛО РАЗНООБРАЗИЯ: три поста (telegram/vk/instagram) должны ОТКРЫВАТЬСЯ по-разному и брать разные акценты — НЕ повторяй одну первую фразу и одни обороты. НЕ начинай с «Привет, друзья!».\n".
...
function ai_ads($cfg, $c){
  ...
  if ($provider === 'openai') {
    ...
    $temp = isset($cfg['temperature'])?(float)$cfg['temperature']:0.9;
...
if (isset($_GET['action'])) {
  ...
  if ($a==='save'){
    ...
    foreach($cars as $c){ if(!empty($c['publish'])){
      ...
      if($cover!==''){ $ph=array_values(array_filter($ph,fn($u)=>$u!==$cover)); array_unshift($ph,$cover); }
  ...
  if ($a==='video_up'){
    if(empty($_FILES['file'])||!is_uploaded_file($_FILES['file']['tmp_name'])){ echo json_encode(['ok'=>false,'err'=>'файл не получен (возможно, больше лимита хостинга)']); exit; }
    $f=$_FILES['file'];
    if($f['size']>80*1024*1024){ echo json_encode(['ok'=>false,'err'=>'видео больше 80 МБ']); exit; }
    $ext=strtolower(pathinfo((string)$f['name'],PATHINFO_EXTENSION));
    if(!in_array($ext,['mp4','mov','webm','m4v'])){ echo json_encode(['ok'=>false,'err'=>'формат: mp4/mov/webm']); exit; }
    $dir=__DIR__.'/uploads'; if(!is_dir($dir)) @mkdir($dir,0755,true);
    if(!is_dir($dir)){ echo json_encode(['ok'=>false,'err'=>'нет папки uploads']); exit; }
    $fn='vid_'.bin2hex(random_bytes(6)).'.'.$ext;
...
<div class="wrap">
  ...
  <div id="adModal" style="position:fixed;inset:0;background:rgba(30,20,12,.5);z-index:50;display:none;place-items:center;padding:16px">
    <div class="card" style="width:min(700px,96vw);max-height:92vh;overflow:auto;margin:0">
      ...
      <div style="margin-bottom:12px">
        <label style="font-size:12px;color:var(--muted);display:block;margin-bottom:4px">Направление / акцент объявления (необязательно)</label>
        <input id="ad_brief" placeholder="напр.: упор на историю с конкурентами и таможню; эмоционально; технично…" style="width:100%">
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          <button class="btn-ghost btn-sm" type="button" onclick="setBrief('Сделай акцент на истории с конкурентами и таможней — с интригой')">Про конкурентов</button>
          <button class="btn-ghost btn-sm" type="button" onclick="setBrief('По-человечески, эмоционально, про доверие и семью')">Эмоционально</button>
          <button class="btn-ghost btn-sm" type="button" onclick="setBrief('Технично, по фактам и характеристикам, для тех кто разбирается')">По фактам</button>
          <button class="btn-ghost btn-sm" type="button" onclick="setBrief('Коротко и цепко: минимум текста, максимум крючок')">Кратко</button>
        </div>
      </div>
  ...
  <div id="pane-cars">
    <div class="card"><div class="frm">
      ...
      <div style="grid-column:span 2"><label>Фото авто (можно несколько — превью и реклама; ★ — обложка лендинга)</label><input id="c_photo" type="file" accept="image/*" multiple onchange="pickPhotos(this)"><div id="c_photo_prev" style="display:none;margin-top:6px"></div></div>
      <div style="grid-column:span 2"><label>Видео авто (короткие ролики — для соцсетей, mp4/mov)</label><input id="c_video" type="file" accept="video/*" multiple onchange="pickVideos(this)"><div id="c_video_prev" style="display:none;margin-top:6px"></div></div>
  ...
  <div id="pane-exp" class="hide">
    <div class="card"><div class="frm" style="grid-template-columns:repeat(3,1fr)">
      <div><label>Дата</label><input id="e_date" type="date" onchange="expCbr()"></div>
      <div><label>Категория</label><select id="e_cat"></select></div>
      <div><label>Валюта</label><select id="e_cur" onchange="expCurChange()"></select></div>
      <div><label>Сумма</label><input id="e_amount" type="number" min="0" step="0.01" placeholder="0" oninput="expCalc()"></div>
      <div id="e_rate_wrap"><label>Курс, ₽ за 1</label><input id="e_rate" type="number" min="0" step="0.0001" placeholder="0" oninput="expCalc()"><div id="e_cbr" style="font-size:11.5px;color:var(--muted);margin-top:4px"></div></div>
      <div><label>Итого, ₽</label><input id="e_rub" readonly style="background:var(--bg);font-weight:700"></div>
      <div style="grid-column:span 2"><label>Комментарий</label><input id="e_note" placeholder="за что"></div>
      <div><button class="btn" style="width:100%" onclick="addExp()">Добавить расход</button></div>
    </div></div>
    <div class="card tbl-wrap"><table><thead><tr><th>Дата</th><th>Категория</th><th class="num">Сумма</th><th>Вал.</th><th class="num">Курс</th><th class="num">₽</th><th>Комментарий</th><th></th></tr></thead><tbody id="exp-body"></tbody></table></div>
  </div>
...
function renderCars(){const b=document.getElementById('cars-body');b.innerHTML='';DATA.cars.forEach(c=>{const p=carProfit(c);const ps=carPhotos(c);const thumb=ps.length?`<img src="${esc(ps[0])}" alt="" style="height:30px;width:46px;object-fit:cover;border-radius:5px;vertical-align:middle;margin-right:7px">`:'';const more=ps.length>1?` <span style="font-size:11px;color:var(--faint)">+${ps.length-1}</span>`:'';const vids=carVideos(c).length?` <span style="font-size:11px;color:var(--faint)">🎬${carVideos(c).length}</span>`:'';const pub=`<button class="btn-ghost btn-sm" onclick="togglePublish('${c.id}')" title="${c.publish?'На лендинге — нажмите, чтобы скрыть':'Скрыто — нажмите, чтобы показать на лендинге'}" style="${c.publish?'background:var(--accent);color:#fff;border-color:var(--accent)':'opacity:.55'}">🌐</button>`;const tr=document.createElement('tr');tr.innerHTML=`<td>${thumb}${esc(c.name)}${more}${vids}</td><td>${esc(c.country||'')}</td><td>${esc(c.date||'')}</td><td class="num">${fmt(carCosts(c))}</td><td class="num">${fmt(c.sale)}</td><td class="num ${p>=0?'profit-pos':'profit-neg'}">${fmt(p)}</td><td style="white-space:nowrap;text-align:right"><button class="btn-ghost btn-sm" onclick="openCost('${c.id}')" title="Добавить расход">＋ Расход</button> ${pub} <button class="btn-ghost btn-sm" onclick="genAds('${c.id}')" title="Сгенерировать объявление">✨${c.ads&&(c.ads.telegram||c.ads.vk||c.ads.instagram)?' ✓':''}</button> <button class="btn-ghost btn-sm" onclick="editCar('${c.id}')" title="Редактировать">✎</button> <button class="del" onclick="delCar('${c.id}')" title="Удалить">✕</button></td>`;b.appendChild(tr);});renderSummary();}
function expRub(e){return (e.rub!=null)?(+e.rub||0):curRub(e);}
function renderExp(){const b=document.getElementById('exp-body');b.innerHTML='';DATA.expenses.forEach(e=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(e.date||'')}</td><td>${esc(e.cat||'')}</td><td class="num">${(+e.amount||0).toLocaleString('ru-RU')}</td><td>${esc(e.currency||'RUB')}</td><td class="num">${(e.currency&&e.currency!=='RUB')?(+e.rate||0):'—'}</td><td class="num">${fmt(expRub(e))}</td><td>${esc(e.note||'')}</td><td><button class="del" onclick="delExp('${e.id}')">✕</button></td>`;b.appendChild(tr);});renderSummary();}
function expCurChange(){const isR=document.getElementById('e_cur').value==='RUB';document.getElementById('e_rate_wrap').style.display=isR?'none':'';if(isR){document.getElementById('e_rate').value='';document.getElementById('e_cbr').textContent='';}expCalc();expCbr();}
function expCalc(){const cur=document.getElementById('e_cur').value;const a=+document.getElementById('e_amount').value||0;const r=cur==='RUB'?1:(+document.getElementById('e_rate').value||0);document.getElementById('e_rub').value=fmt(Math.round(a*r));}
let expCbrToken=0;
async function expCbr(){const cur=document.getElementById('e_cur').value;const el=document.getElementById('e_cbr');if(!el)return;if(cur==='RUB'){el.textContent='';return;}const date=document.getElementById('e_date').value;el.textContent='Курс ЦБ…';const my=++expCbrToken;const res=await fetchCbrRate(date,cur);if(my!==expCbrToken)return;if(res){const rv=Math.round(res.rate*10000)/10000;el.innerHTML='Курс ЦБ'+(res.fallback?' (последний)':'')+': <b>'+String(rv).replace('.',',')+' ₽</b> · <a href="#" style="color:var(--accent)" onclick="document.getElementById(\'e_rate\').value='+rv+';expCalc();return false;">подставить</a>';}else el.textContent='Курс ЦБ недоступен — впишите вручную';}
function renderSummary(){const cp=DATA.cars.reduce((s,c)=>s+carProfit(c),0);const ex=DATA.expenses.reduce((s,e)=>s+expRub(e),0);const net=cp-ex;document.getElementById('summary').innerHTML=`<div class="stat"><span>Машин в учёте</span><b>${DATA.cars.length}</b></div><div class="stat good"><span>Прибыль с машин</span><b>${fmt(cp)}</b></div><div class="stat bad"><span>Общие расходы</span><b>${fmt(ex)}</b></div><div class="stat ${net>=0?'acc':'bad'}"><span>Чистая прибыль</span><b>${fmt(net)}</b></div>`;}
let editingCarId=null, pendingPhotos=[], editingPhotos=[], editingCover='';
...
function renderPhotoPrev(){const box=document.getElementById('c_photo_prev');box.innerHTML='';const mk=(src,onx,coverUrl)=>{const w=document.createElement('span');w.style.cssText='position:relative;display:inline-block;margin:4px 10px 10px 0';const isCover=coverUrl&&editingCover===coverUrl;const im=document.createElement('img');im.src=src;im.style.cssText='height:54px;width:74px;object-fit:cover;border-radius:6px;display:block;border:2px solid '+(isCover?'var(--accent)':'var(--line)');const b=document.createElement('button');b.textContent='✕';b.type='button';b.title='Удалить';b.style.cssText='position:absolute;top:-7px;right:-7px;background:#c0432c;color:#fff;border:0;border-radius:50%;width:19px;height:19px;font-size:11px;line-height:1;cursor:pointer;padding:0';b.onclick=onx;w.appendChild(im);w.appendChild(b);if(coverUrl){const st=document.createElement('button');st.type='button';st.textContent=isCover?'★ на лендинг':'☆';st.title='Сделать обложкой для лендинга';st.style.cssText='position:absolute;bottom:-9px;left:50%;transform:translateX(-50%);white-space:nowrap;background:'+(isCover?'var(--accent)':'var(--card)')+';color:'+(isCover?'#fff':'var(--muted)')+';border:1px solid var(--line);border-radius:9px;font-size:10px;line-height:1;cursor:pointer;padding:3px 6px';st.onclick=()=>{editingCover=coverUrl;renderPhotoPrev();};w.appendChild(st);}box.appendChild(w);};editingPhotos.forEach((u,i)=>mk(u,()=>{editingPhotos.splice(i,1);if(editingCover===u)editingCover=editingPhotos[0]||'';renderPhotoPrev();},u));pendingPhotos.forEach((d,i)=>mk(d,()=>{pendingPhotos.splice(i,1);renderPhotoPrev();},null));box.style.display=(editingPhotos.length||pendingPhotos.length)?'block':'none';}
let formVideos=[];
function carVideos(c){return (c.videos&&c.videos.length)?c.videos:[];}
function vidSrc(u){u=String(u);if(/^https?:/i.test(u))return u;return u.replace(/^\/uchet\//,'').replace(/^\//,'');}
function renderVideoPrev(){const box=document.getElementById('c_video_prev');box.innerHTML='';formVideos.forEach((u,i)=>{const w=document.createElement('span');w.style.cssText='position:relative;display:inline-block;margin:4px 12px 6px 0;vertical-align:top';const v=document.createElement('video');v.src=vidSrc(u);v.muted=true;v.setAttribute('preload','metadata');v.setAttribute('playsinline','');v.style.cssText='height:70px;border-radius:6px;display:block;border:1px solid var(--line);background:#000';const b=document.createElement('button');b.type='button';b.textContent='✕';b.title='Удалить';b.style.cssText='position:absolute;top:-7px;right:-7px;background:#c0432c;color:#fff;border:0;border-radius:50%;width:19px;height:19px;font-size:11px;line-height:1;cursor:pointer;padding:0';b.onclick=()=>{formVideos.splice(i,1);renderVideoPrev();};w.appendChild(v);w.appendChild(b);box.appendChild(w);});box.style.display=formVideos.length?'block':'none';}
async function pickVideos(inp){const fs=Array.from(inp.files||[]);const box=document.getElementById('c_video_prev');box.style.display='block';for(const f of fs){const st=document.createElement('div');st.className='muted';st.style.fontSize='12px';st.textContent='Загрузка видео '+f.name+'…';box.appendChild(st);try{const fd=new FormData();fd.append('file',f);const r=await fetch('?action=video_up',{method:'POST',body:fd});const j=await r.json();st.remove();if(j.ok)formVideos.push(j.url);else alert('Видео не загрузилось: '+(j.err||''));}catch(e){st.remove();alert('Видео не загрузилось: '+e.message);}}inp.value='';renderVideoPrev();}
function clearCarForm(){const g=id=>document.getElementById(id);['c_name','c_sale','c_note'].forEach(i=>g(i).value='');g('c_date').value=new Date().toISOString().slice(0,10);g('c_photo').value='';g('c_video').value='';g('c_publish').checked=false;pendingPhotos=[];editingPhotos=[];editingCover='';formVideos=[];renderPhotoPrev();renderVideoPrev();}
...
async function addCar(){const g=id=>document.getElementById(id);if(!g('c_name').value.trim()){g('c_name').focus();return;}const d=carFormData();const btn=g('carBtn');let urls=[];if(pendingPhotos.length){const t0=btn.textContent;btn.textContent='Загрузка фото…';for(const dp of pendingPhotos){const r=await api('photo_up',{data:dp});if(!r.ok){alert('Фото не загрузилось: '+(r.err||''));btn.textContent=t0;return;}urls.push(r.url);}btn.textContent=t0;}const photos=editingPhotos.concat(urls);const cover=(editingCover&&photos.includes(editingCover))?editingCover:(photos[0]||'');if(editingCarId){const c=DATA.cars.find(x=>x.id===editingCarId);if(c){Object.assign(c,d);c.photos=photos;c.cover=cover;c.videos=formVideos.slice();delete c.photo;}editingCarId=null;btn.textContent='Добавить авто';}else{const car={id:uid(),...d};if(photos.length){car.photos=photos;car.cover=cover;}if(formVideos.length)car.videos=formVideos.slice();DATA.cars.unshift(car);}clearCarForm();renderCars();save();}
...
function editCar(id){const c=DATA.cars.find(x=>x.id===id);if(!c)return;const el=x=>document.getElementById(x);el('c_name').value=c.name||'';el('c_country').value=c.country||'Китай';el('c_date').value=c.date||'';el('c_sale').value=c.sale||'';el('c_note').value=c.note||'';el('c_publish').checked=!!c.publish;pendingPhotos=[];editingPhotos=carPhotos(c).slice();editingCover=c.cover||(editingPhotos[0]||'');formVideos=(c.videos||[]).slice();el('c_photo').value='';el('c_video').value='';renderPhotoPrev();renderVideoPrev();editingCarId=id;el('carBtn').textContent='Сохранить изменения';window.scrollTo({top:0,behavior:'smooth'});}
