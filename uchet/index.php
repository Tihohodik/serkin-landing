...
function clearCarForm(){const g=id=>document.getElementById(id);['c_name','c_sale','c_note'].forEach(i=>g(i).value='');g('c_date').value=new Date().toISOString().slice(0,10);g('c_photo').value='';g('c_publish').checked=false;pendingPhotos=[];editingPhotos=[];editingCover='';renderPhotoPrev();}
...
function editCar(id){const c=DATA.cars.find(x=>x.id===id);if(!c)return;const el=x=>document.getElementById(x);el('c_name').value=c.name||'';el('c_country').value=c.country||'Китай';el('c_date').value=c.date||'';el('c_sale').value=c.sale||'';el('c_note').value=c.note||'';el('c_publish').checked=!!c.publish;pendingPhotos=[];editingPhotos=carPhotos(c).slice();editingCover=c.cover||(editingPhotos[0]||'');el('c_photo').value='';renderPhotoPrev();editingCarId=id;el('carBtn').textContent='Сохранить изменения';window.scrollTo({top:0,behavior:'smooth'});}
