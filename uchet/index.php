  ...
  <div id="costModal" style="position:fixed;inset:0;background:rgba(30,20,12,.5);z-index:50;display:none;place-items:center;padding:16px">
    <div class="card" style="width:min(720px,96vw);max-height:92vh;overflow:auto;margin:0">
      ...
      <div class="frm" style="grid-template-columns:repeat(3,1fr)">
        ...
        <div><label>Дата оплаты</label><input id="ct_date" type="date"></div>
        ...
        <div id="ct_rate_wrap"><label>Курс, ₽ за 1</label><input id="ct_rate" type="number" min="0" step="0.0001" placeholder="0" oninput="costCalc()"></div>
        ...
        <div><label>&nbsp;</label><button class="btn" style="width:100%" onclick="addCostLine()">＋ Добавить</button></div>
      </div>
...
function renderCars(){const b=document.getElementById('cars-body');b.innerHTML='';DATA.cars.forEach(c=>{const p=carProfit(c);const ps=carPhotos(c);const thumb=ps.length?`<img src="${esc(ps[0])}" alt="" style="height:30px;width:46px;object-fit:cover;border-radius:5px;vertical-align:middle;margin-right:7px">`:'';const more=ps.length>1?` <span style="font-size:11px;color:var(--faint)">+${ps.length-1}</span>`:'';const pub=`<button class="btn-ghost btn-sm" onclick="togglePublish('${c.id}')" title="Публикация на лендинге" style="${c.publish?'background:var(--accent);color:#fff;border-color:var(--accent)':''}">🌐 ${c.publish?'на сайте':'скрыто'}</button>`;const tr=document.createElement('tr');tr.innerHTML=`<td>${thumb}${esc(c.name)}${more}</td><td>${esc(c.country||'')}</td><td>${esc(c.date||'')}</td><td class="num">${fmt(costByType(c,'Закупка'))}</td><td class="num">${fmt(costByType(c,'Логистика'))}</td><td class="num">${fmt(costByType(c,'Растаможка'))}</td><td class="num">${fmt(costByType(c,'Утиль'))}</td><td class="num">${fmt(costByType(c,'Прочее'))}</td><td class="num">${fmt(carCosts(c))}</td><td class="num">${fmt(c.sale)}</td><td class="num ${p>=0?'profit-pos':'profit-neg'}">${fmt(p)}</td><td style="white-space:nowrap"><button class="btn-ghost btn-sm" onclick="openCost('${c.id}')">＋ Расход</button> ${pub} <button class="btn-ghost btn-sm" onclick="editCar('${c.id}')">✎</button> <button class="btn-ghost btn-sm" onclick="genAds('${c.id}')">✨ Текст${c.ads&&(c.ads.telegram||c.ads.vk||c.ads.instagram)?' ✓':''}</button> <button class="del" onclick="delCar('${c.id}')">✕</button></td>`;b.appendChild(tr);});renderSummary();}
...
let costCarId=null;
function openCost(id){const c=DATA.cars.find(x=>x.id===id);if(!c)return;if(!Array.isArray(c.costs))c.costs=[];costCarId=id;document.getElementById('cost_title').textContent='Расходы · '+c.name;document.getElementById('ct_type').innerHTML=COST_TYPES.map(x=>`<option>${x}</option>`).join('');document.getElementById('ct_cur').innerHTML=CUR.map(p=>`<option value="${p[0]}">${p[1]}</option>`).join('');document.getElementById('ct_date').value=new Date().toISOString().slice(0,10);document.getElementById('ct_amount').value='';document.getElementById('ct_rate').value='';document.getElementById('ct_note').value='';costCurChange();renderCostRows();document.getElementById('costModal').style.display='grid';}
function closeCost(){document.getElementById('costModal').style.display='none';costCarId=null;}
function costCurChange(){const isR=document.getElementById('ct_cur').value==='RUB';document.getElementById('ct_rate_wrap').style.display=isR?'none':'';if(isR)document.getElementById('ct_rate').value='';costCalc();}
...
function renderCostRows(){const c=DATA.cars.find(x=>x.id===costCarId);if(!c)return;const b=document.getElementById('cost-body');b.innerHTML='';(c.costs||[]).forEach(x=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${esc(x.type)}</td><td>${esc(x.date||'')}</td><td class="num">${(+x.amount||0).toLocaleString('ru-RU')}</td><td>${esc(x.currency||'RUB')}</td><td class="num">${(x.currency&&x.currency!=='RUB')?(+x.rate||0):'—'}</td><td class="num">${fmt(x.rub)}</td><td><button class="del" onclick="delCostLine('${x.id}')">✕</button></td>`;b.appendChild(tr);});document.getElementById('cost-total').textContent=fmt(carCosts(c));}
function addCostLine(){const c=DATA.cars.find(x=>x.id===costCarId);if(!c)return;const a=+document.getElementById('ct_amount').value||0;if(!a){document.getElementById('ct_amount').focus();return;}const cur=document.getElementById('ct_cur').value;const rate=cur==='RUB'?1:(+document.getElementById('ct_rate').value||0);if(cur!=='RUB'&&!rate){document.getElementById('ct_rate').focus();return;}const item={id:uid(),type:document.getElementById('ct_type').value,date:document.getElementById('ct_date').value,amount:a,currency:cur,rate:rate,note:document.getElementById('ct_note').value.trim()};item.rub=curRub(item);if(!Array.isArray(c.costs))c.costs=[];c.costs.push(item);document.getElementById('ct_amount').value='';document.getElementById('ct_note').value='';costCalc();renderCostRows();renderCars();save();}
function delCostLine(cid){const c=DATA.cars.find(x=>x.id===costCarId);if(!c)return;c.costs=(c.costs||[]).filter(x=>x.id!==cid);renderCostRows();renderCars();save();}
